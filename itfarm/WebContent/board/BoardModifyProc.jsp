...
try {
	...
	if(passwd.equals(dbPass)) {
		...
		if(filename != null) { 		// ------ 새로운 업로드 파일이 존재하는 경우
			...
			//------------------------------- 파일정보를 포함한 데이터베이스 갱신
			String Query2 = "UPDATE board1 SET  UsrSubject=?, UsrContent=?, UsrFileName=?, UsrFileSize=? WHERE RcdNo=?";
			...
			String Query2 = "UPDATE board1 SET  UsrSubject=?, UsrContent=? WHERE RcdNo=?";
			pstmt = conn.prepareStatement(Query2);
			pstmt.setString(1, subject);
			pstmt.setString(2, content);
			pstmt.setInt(3, rno);
