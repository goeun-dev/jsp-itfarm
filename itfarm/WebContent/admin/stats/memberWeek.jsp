...
Map<String, Object> annualRevenue = (Map<String, Object>)request.getAttribute("weekRevenue");
Map<String, Object> annualRfRevenue = (Map<String, Object>)request.getAttribute("weekRefundRevenue");
%>
<html>
<title>회원통계</title>
<meta charset="utf-8">
...
function drawChart() {
    ...
    linedata.addRows([
  		['1주', <%= annualRfRevenue.get("member_01") %>, <%= annualRevenue.get("member_01") %>],
        ['2주', <%= annualRfRevenue.get("member_02") %>, <%= annualRevenue.get("member_02") %>],
        ['3주', <%= annualRfRevenue.get("member_03") %>, <%= annualRevenue.get("member_03") %>],
        ['4주', <%= annualRfRevenue.get("member_04") %>, <%= annualRevenue.get("member_04") %>],
        ['5주', <%= annualRfRevenue.get("member_05") %>, <%= annualRevenue.get("member_05") %>],
        ['6주', <%= annualRfRevenue.get("member_06") %>, <%= annualRevenue.get("member_06") %>],
        ['7주', <%= annualRfRevenue.get("member_07") %>, <%= annualRevenue.get("member_07") %>]
  		]);

    var lineOpts = {
      chart: {
        title: '최근 <%=week%> 주 가입자수 현황',
        subtitle: ''
      },
      vAxis: {
    	  format: 'decimal'
                    ...
                    <div class="panel panel-default">
						...
						<br>
							<div class="btn-group" role="group" aria-label="버튼 그룹">
								...
								class="btn btn-primary" type="submit">주별</button>
