...
function drawChart() {
    ...
    linedata.addRows([
  		...
  		['1월', <%= annualRfRevenue.get("reserv_price_01") %>, <%= annualRevenue.get("reserv_price_01") %>],
        ['2월', <%= annualRfRevenue.get("reserv_price_02") %>, <%= annualRevenue.get("reserv_price_02") %>],
        ['3월', <%= annualRfRevenue.get("reserv_price_03") %>, <%= annualRevenue.get("reserv_price_03") %>],
        ['4월', <%= annualRfRevenue.get("reserv_price_04") %>, <%= annualRevenue.get("reserv_price_04") %>],
        ['5월', <%= annualRfRevenue.get("reserv_price_05") %>, <%= annualRevenue.get("reserv_price_05") %>],
        ['6월', <%= annualRfRevenue.get("reserv_price_06") %>, <%= annualRevenue.get("reserv_price_06") %>],
        ['7월', <%= annualRfRevenue.get("reserv_price_07") %>, <%= annualRevenue.get("reserv_price_07") %>],
        ['8월', <%= annualRfRevenue.get("reserv_price_08") %>, <%= annualRevenue.get("reserv_price_08") %>],
        ['9월', <%= annualRfRevenue.get("reserv_price_09") %>, <%= annualRevenue.get("reserv_price_09") %>],
        ['10월', <%= annualRfRevenue.get("reserv_price_10") %>, <%= annualRevenue.get("reserv_price_10") %>],
        ['11월', <%= annualRfRevenue.get("reserv_price_11") %>, <%= annualRevenue.get("reserv_price_11") %>],
        ['12월', <%= annualRfRevenue.get("reserv_price_12") %>, <%= annualRevenue.get("reserv_price_12") %>]
  		]);

    var lineOpts = {
      chart: {
        title: '2018 년 월별 예약매출현황',
        subtitle: ''
      },
      vAxis: {
    	  format: 'decimal'
                    ...
                    <div class="panel panel-default">
						...
						<br>
							<div class="btn-group" role="group" aria-label="버튼 그룹">
								...
								class="btn btn-primary btn-sm" type="submit">월별</button>
