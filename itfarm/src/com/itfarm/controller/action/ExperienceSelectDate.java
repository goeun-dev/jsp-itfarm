...
public class ExperienceSelectDate implements Action {

	public ExperienceSelectDate() {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String urla = "experience/experienceDetail.jsp";
		
		String prodCode = request.getParameter("prodCode");
		String selectDate = request.getParameter("selectDate");
		int stdCnt2 = Integer.parseInt(request.getParameter("stdCnt"));
		System.out.println("선택날짜"+selectDate);
		ExperienceDAO eDao = ExperienceDAO.getInstance();
		List<ReservationVO> reservationList = eDao.reservationNum(prodCode, stdCnt2);
		request.setAttribute("reservationList", reservationList);
		request.setAttribute("selectDate", selectDate);
		response.setCharacterEncoding("EUC-KR");
		PrintWriter writer = response.getWriter();
		writer.println("<script type='text/javascript'>");
		writer.println("alert('선택되었습니다.');");
